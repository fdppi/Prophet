...
#define RUN_TIME(variable, ...) do{		\
	...
	variable += (double) (end.tv_usec - start.tv_usec)  + 	\
...
#define SEQ_BLOCK_NUM 3


typedef struct _tb_info_{
    uint64_t tb_addr;
    uint64_t use_count;
    uint64_t former_tb_addr;
    struct _tb_info_ *next;
    uint64_t former_combine_addr;
    uint64_t former_combine_addr2;
    uint64_t *BBV;
    uint64_t *former_BBV;
    ...
    uint64_t m_local_fetch_count;
    uint64_t m_local_size_count;
    uint64_t m_local_cycles;
    uint64_t m_initial_local_cycles;
    ...
    uint64_t m_block_retire_buf[BLOCK_INSTRUCTION];
    uint64_t addr[BLOCK_INSTRUCTION];
    int m_next[BLOCK_INSTRUCTION];
...
#define TB_CACHE_BITS 15
#define TB_CACHE_SIZE 1<<TB_CACHE_BITS
#else
    #define SEQ_BLOCK_NUM 1
#endif
 ...
 */
//14 instruction-types supported
enum fu_type_t {
...
struct inorder_retire_info {
	int inorder_uop_sum; //multiple uop instrucitons map to a single x86 instruction
...
public:
...
#ifdef JXF_BLOCK
    tb_info_struct_t *tb_info_temp;
    uint64_t former_tb_addr;
    uint64_t former_combine_addr;
    uint64_t former_combine_addr2;
    bool former_dup;
    //[2x: cycle when inst commit 2x+1:inst no] 
    int block_retire_ptr = 0;
    uint64_t base_cycle;
    
    void *current_tb_array[GATHER_BLOCK_NUM];  // for gather blocks 
#endif    
    uint64_t search_count;// count for tb counted
    uint64_t block_nu;
    uint64_t *BBV;
    uint64_t *former_BBV;
	...
	int *m_next_fu_type;
	struct inorder_retire_info m_inorder_x86_retire_info;//always keep information of the last retired instruction 
...
#ifdef JXF_BLOCK
    int find_duplicate(uint64_t tb_addr,tb_info_struct_t **tb_info);
    void count_duplicate(tb_info_struct_t *tb_info);
#endif    
