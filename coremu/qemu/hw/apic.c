...
void cm_apic_set_irq(DeviceState *d, int vector_num, int trigger_mode)
{
    APICState *s = DO_UPCAST(APICState, busdev.qdev, d);
    apic_set_irq(s, vector_num, trigger_mode);
}

void cm_apic_startup(DeviceState *d, int vector_num)
{
    APICState *s = DO_UPCAST(APICState, busdev.qdev, d);
    apic_startup(s, vector_num);
}

void cm_apic_setup_arbid(DeviceState *d)
{
    APICState *s = DO_UPCAST(APICState, busdev.qdev, d);
    s->arb_id = s->id;
}
