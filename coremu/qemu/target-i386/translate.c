...
{                                                                                   \ 
    (tcg_gen_qemu_ld8u)(arg, addr, mem_index);                                      \
        gen_helper_trifs_trace_tcg_mem_ld_ubyte_after(                              \
             tcg_const_tl(ana_helpler_record_pc), addr);  \
}

#define tcg_gen_qemu_ld16u(arg, addr, mem_index)                                    \
{                                                                                   \
    (tcg_gen_qemu_ld16u)(arg, addr, mem_index);                                     \
        gen_helper_trifs_trace_tcg_mem_ld_uword_after(                              \
             tcg_const_tl(ana_helpler_record_pc), addr);  \
}
...
{                                                                           \
    (tcg_gen_qemu_ld32u)(arg, addr, mem_index);                                     \
        gen_helper_trifs_trace_tcg_mem_ld_ulong_after(                              \
             tcg_const_tl(ana_helpler_record_pc), addr);  \
}
...
{                                                                               \
    (tcg_gen_qemu_ld8s)(arg, addr, mem_index);                                      \
        gen_helper_trifs_trace_tcg_mem_ld_sbyte_after(                              \
             tcg_const_tl(ana_helpler_record_pc), addr);  \
}
...
{                                                                              \
    (tcg_gen_qemu_ld16s)(arg, addr, mem_index);                                     \
        gen_helper_trifs_trace_tcg_mem_ld_sword_after(                              \
             tcg_const_tl(ana_helpler_record_pc), addr);  \
}
...
{                                                                               \
    (tcg_gen_qemu_ld32s)(arg, addr, mem_index);                                     \
        gen_helper_trifs_trace_tcg_mem_ld_slong_after(                              \
             tcg_const_tl(ana_helpler_record_pc), addr);  \
}
#endif                                                                              
...
{                                                                               \
    (tcg_gen_qemu_ld64)(arg, addr, mem_index);                                      \
        gen_helper_trifs_trace_tcg_mem_ld_quad_after(                               \
             tcg_const_tl(ana_helpler_record_pc), addr);  \
}
...
{                                                                               \
    (tcg_gen_qemu_st8)(arg, addr, mem_index);                                       \
        gen_helper_trifs_trace_tcg_mem_st_byte_after(                               \
             tcg_const_tl(ana_helpler_record_pc), addr);   \
}
...
{                                                                               \
    (tcg_gen_qemu_st16)(arg, addr, mem_index);                                      \
        gen_helper_trifs_trace_tcg_mem_st_word_after(                               \
             tcg_const_tl(ana_helpler_record_pc), addr);   \
}
...
{                                                                               \
    (tcg_gen_qemu_st32)(arg, addr, mem_index);                                      \
        gen_helper_trifs_trace_tcg_mem_st_long_after(                               \
            tcg_const_tl(ana_helpler_record_pc), addr);   \
}
...
{                                                                              \
    (tcg_gen_qemu_st64)(arg, addr, mem_index);                                      \
        gen_helper_trifs_trace_tcg_mem_st_quad_after(                               \
            tcg_const_tl(ana_helpler_record_pc), addr);   \
}
#endif
...
//#define COREMU_LOCK_DETECT

#if defined(CONFIG_COREMU) && defined(COREMU_LOCK_DETECT)
/* lock detection */
...
struct timeval ana_begin;
struct timeval ana_end;
...
//#define minqh_timer
//#define minqh_timer_count

#ifdef minqh_timer
extern void m_stop_all_timers();
#endif
...
static target_ulong disas_insn(DisasContext *s, target_ulong pc_start)
{
 ...
 reswitch:
    ...
    case 0xcd: /* int N */
...
#if defined(CONFIG_COREMU) && defined(COREMU_DEBUG_MODE) //#1  FALSE
            if (val == 0x86)
...
#else //#1
	#ifdef ANALYTICAL_MODEL_INSTRUMENT  //#2 TRUE
		if(val == 0x86){
		#ifdef ANALYTICAL_MODEL_TRACE_LOG 
			ftrifs_trace_log("0x86 appears!");
		#endif
			if(!analytical_model_instrument_flag){  // first 0x86
				analytical_model_instrument_flag = 1;
                #ifdef minqh_timer
                m_stop_all_timers();
                #endif
				printf("first magic break\n");
				fflush(stdout);
				gettimeofday(&ana_begin, NULL);
			}else{ 									//second 0x86
				printf("second magic break\n");
				/*barrier_end_notification(cpu_single_env->cpu_index);*/
				analytical_model_instrument_flag = 0;
				gettimeofday(&ana_end, NULL);
                    		printf("consumed time is %lf (s)\n", 
				((double)ana_end.tv_usec / 1000000.0 + (double)ana_end.tv_sec) - 
				((double)ana_begin.tv_usec / 1000000.0 + (double)ana_begin.tv_sec));
				fflush(stdout);

		#ifdef ANALYTICAL_MODEL_STATISTICS //#3
				uint64_t print_analytical_all_user_instr_num = 0;
				uint64_t print_analytical_all_kernel_instr_num = 0;
				uint64_t print_analytical_all_total_instr_num = 0;

                #ifdef minqh_timer_count
                uint64_t ana_interrupt_count = 0;
                uint64_t ana_ipi_count = 0;
                uint64_t ana_ipi_send_count = 0;
                #endif
				int ana_core_index = 0;
				int total_max_basic = 0;
				printf("max_basic_block: ");
				for(;ana_core_index < ANALYTICAL_COREMU_TOTAL_CORE; ana_core_index++){
					//printf("%d ", analytical_max_basic_block_instr_num[ana_core_index]);
					if(analytical_max_basic_block_instr_num[ana_core_index] > total_max_basic){
						total_max_basic = analytical_max_basic_block_instr_num[ana_core_index];
					}
				}
				printf("\n");
				printf("total_max_basic_block: %d\n", total_max_basic);

				ana_core_index = 0;
				for(;ana_core_index < ANALYTICAL_COREMU_TOTAL_CORE; ana_core_index++){
					print_analytical_all_user_instr_num += analytical_user_instruction_num[ana_core_index];
					print_analytical_all_kernel_instr_num += analytical_kernel_instruction_num[ana_core_index];
					print_analytical_all_total_instr_num = print_analytical_all_total_instr_num + analytical_user_instruction_num[ana_core_index] + analytical_kernel_instruction_num[ana_core_index];
                    #ifdef minqh_timer_count
                    extern uint64_t analytical_interrupt_count[ANALYTICAL_COREMU_TOTAL_CORE];
                    extern uint64_t analytical_ipi_count[ANALYTICAL_COREMU_TOTAL_CORE];
                    extern uint64_t analytical_ipi_send_count[ANALYTICAL_COREMU_TOTAL_CORE];
                    ana_interrupt_count += analytical_interrupt_count[ana_core_index];
                    ana_ipi_count += analytical_ipi_count[ana_core_index];
                    ana_ipi_send_count += analytical_ipi_send_count[ana_core_index];
                    #endif
				}
				printf("analytical_all_user_instr_num: %ld\n", print_analytical_all_user_instr_num);
				printf("analytical_all_kenerl_instr_num: %ld\n", print_analytical_all_kernel_instr_num);
				printf("analytical_all_total_instr_num: %ld\n", print_analytical_all_total_instr_num);

                #ifdef minqh_timer_count
                printf("analytical_interrupt_num: %lld\n", ana_interrupt_count);
                printf("analytical_ipi_num: %lld\n", ana_ipi_count);
                printf("analytical_ipi_send_num: %lld\n", ana_ipi_send_count);
                #endif

			#ifdef ANALYTICAL_MODEL_TIMING_SHIFT 
                printf("duplicate correct %lld\ntotal tb %lld\n",dup_tb,total_tb);
                printf("calling tb %lld\nnew_tb %lld\n",call_tb,new_tb);
				timing_end();
			#endif 
		#endif//#3
				exit(0);
		...
		#ifdef ANALYTICAL_MODEL_TRACE_LOG
			ftrifs_trace_log("0x87 appears!");
		#endif
			if(!analytical_model_instrument_flag_before_magic_break){
				analytical_model_instrument_flag_before_magic_break = 1;
			}
			else{
				analytical_model_instrument_flag_before_magic_break = 0;
			}
		}
	#ifdef ALEX_TRANS
		else if(val == 0x89){
			analytical_model_instrument_flag = 1;
			/*trans_begin(cpu_single_env->cpu_index);*/
		}
		else if(val == 0x90){
			/*trans_end(cpu_single_env->cpu_index);*/
			analytical_model_instrument_flag = 0;
		}else if(val == 0x91){
			/*trans_print_info();*/
		}
...
#include "../trifs/translate_inline.h"
#endif

//easton added
//weizao de func
void easton_gen_lea_modrm(DisasContext *s, int modrm, int *reg_ptr, int *offset_ptr)
{
    target_long disp;
    int havesib;
    int base;
...
//easton added
//weizao de helper func
void easton_gen_ldst_modrm(DisasContext *s, int modrm, int ot, int reg, int is_store)
{
    int mod, rm, opreg, disp;
    ...
    if (mod == 3) {
        ...
        easton_gen_lea_modrm(s, modrm, &opreg, &disp);
        if (is_store) {
            //if (reg != OR_TMP0)
                //gen_op_mov_TN_reg(ot, 0, reg);
            //gen_op_st_T0_A0(ot + s->mem_index);
...
//easton added
//weizao de func
void easton_gen_sse(DisasContext *s, int b, target_ulong pc_start, int rex_r)
{
    int b1, op1_offset, op2_offset, is_xmm, val, ot;
    int modrm, mod, rm, reg, reg_addr, offset_addr;
    void *sse_op2;
    ...
    if (sse_op2 == SSE_SPECIAL) {
        ...
        case 0x0e7: /* movntq */
            if (mod == 3)
                goto illegal_op;
            easton_gen_lea_modrm(s, modrm, &reg_addr, &offset_addr);
            //gen_stq_env_A0(s->mem_index, offsetof(CPUX86State,fpregs[reg].mmx));
            break;
        case 0x1e7: /* movntdq */
        case 0x02b: /* movntps */
        ...
        case 0x32b: /* movntsd */
            ...
            if (b1 & 1) {
                ...
                //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,
                    //xmm_regs[reg].XMM_L(0)));
                //gen_op_st_T0_A0(OT_LONG + s->mem_index);
            }
            break;
...
#ifdef TARGET_X86_64
            if (s->dflag == 2) {
                easton_gen_ldst_modrm(s, modrm, OT_QUAD, OR_TMP0, 0);
                //tcg_//gen_st_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,fpregs[reg].mmx));
            } else
#endif
            {
                easton_gen_ldst_modrm(s, modrm, OT_LONG, OR_TMP0, 0);
                //tcg_//gen_addi_ptr(cpu_ptr0, cpu_env,
                                 //offsetof(CPUX86State,fpregs[reg].mmx));
                //tcg_//gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T[0]);
                //gen_helper_movl_mm_T0_mmx(cpu_ptr0, cpu_tmp2_i32);
            }
            break;
        case 0x16e: /* movd xmm, ea */
        ...
        case 0x6f: /* movq mm, ea */
            if (mod != 3) {
                ...
                rm = (modrm & 7);
                //tcg_//gen_ld_i64(cpu_tmp1_i64, cpu_env,
                               //offsetof(CPUX86State,fpregs[rm].mmx));
                //tcg_//gen_st_i64(cpu_tmp1_i64, cpu_env,
                               //offsetof(CPUX86State,fpregs[reg].mmx));
            }
            break;
        case 0x010: /* movups */
        ...
        case 0x210: /* movss xmm, ea */
            if (mod != 3) {
                easton_gen_lea_modrm(s, modrm, &reg_addr, &offset_addr);
                //gen_op_ld_T0_A0(OT_LONG + s->mem_index);
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,xmm_regs[reg].XMM_L(0)));
                //gen_op_movl_T0_0();
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,xmm_regs[reg].XMM_L(1)));
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,xmm_regs[reg].XMM_L(2)));
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,xmm_regs[reg].XMM_L(3)));
            } else {
                rm = (modrm & 7) | REX_B(s);
                //gen_op_movl(offsetof(CPUX86State,xmm_regs[reg].XMM_L(0)),
                            //offsetof(CPUX86State,xmm_regs[rm].XMM_L(0)));
...
#ifdef TARGET_X86_64
            if (s->dflag == 2) {
                //tcg_//gen_ld_i64(cpu_T[0], cpu_env,
                               //offsetof(CPUX86State,fpregs[reg].mmx));
                easton_gen_ldst_modrm(s, modrm, OT_QUAD, OR_TMP0, 1);
            } else
#endif
            {
                //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env,
                                 //offsetof(CPUX86State,fpregs[reg].mmx.MMX_L(0)));
                easton_gen_ldst_modrm(s, modrm, OT_LONG, OR_TMP0, 1);
            }
            break;
        case 0x17e: /* movd ea, xmm */
#ifdef TARGET_X86_64
        ...
        case 0x211: /* movss ea, xmm */
            if (mod != 3) {
                easton_gen_lea_modrm(s, modrm, &reg_addr, &offset_addr);
                //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,xmm_regs[reg].XMM_L(0)));
                //gen_op_st_T0_A0(OT_LONG + s->mem_index);
            } else {
                rm = (modrm & 7) | REX_B(s);
                //gen_op_movl(offsetof(CPUX86State,xmm_regs[rm].XMM_L(0)),
        ...
        case 0x173:
            ...
            if (is_xmm) {
                //gen_op_movl_T0_im(val);
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,xmm_t0.XMM_L(0)));
                //gen_op_movl_T0_0();
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,xmm_t0.XMM_L(1)));
                op1_offset = offsetof(CPUX86State,xmm_t0);
            } else {
                //gen_op_movl_T0_im(val);
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,mmx_t0.MMX_L(0)));
                //gen_op_movl_T0_0();
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,mmx_t0.MMX_L(1)));
                op1_offset = offsetof(CPUX86State,mmx_t0);
            }
            sse_op2 = sse_op_table2[((b - 1) & 3) * 8 + (((modrm >> 3)) & 7)][b1];
            if (!sse_op2)
            ...
            //tcg_//gen_addi_ptr(cpu_ptr0, cpu_env, op2_offset);
            //tcg_//gen_addi_ptr(cpu_ptr1, cpu_env, op1_offset);
            //((void (*)(TCGv_ptr, TCGv_ptr))sse_op2)(cpu_ptr0, cpu_ptr1);
            break;
        case 0x050: /* movmskps */
            rm = (modrm & 7) | REX_B(s);
            //tcg_//gen_addi_ptr(cpu_ptr0, cpu_env,
                             //offsetof(CPUX86State,xmm_regs[rm]));
            //gen_helper_movmskps(cpu_tmp2_i32, cpu_ptr0);
            //tcg_//gen_extu_i32_tl(cpu_T[0], cpu_tmp2_i32);
            //gen_op_mov_reg_T0(OT_LONG, reg);
            break;
        case 0x150: /* movmskpd */
            rm = (modrm & 7) | REX_B(s);
        ...
        case 0x12a: /* cvtpi2pd */
            ...
            op1_offset = offsetof(CPUX86State,xmm_regs[reg]);
            //tcg_//gen_addi_ptr(cpu_ptr0, cpu_env, op1_offset);
            //tcg_//gen_addi_ptr(cpu_ptr1, cpu_env, op2_offset);
            switch(b >> 8) {
            case 0x0:
                //gen_helper_cvtpi2ps(cpu_ptr0, cpu_ptr1);
                break;
        ...
        case 0x32a: /* cvtsi2sd */
            ot = (s->dflag == 2) ? OT_QUAD : OT_LONG;
            easton_gen_ldst_modrm(s, modrm, ot, OR_TMP0, 0);
            op1_offset = offsetof(CPUX86State,xmm_regs[reg]);
            //tcg_//gen_addi_ptr(cpu_ptr0, cpu_env, op1_offset);
            sse_op2 = sse_op_table3[(s->dflag == 2) * 2 + ((b >> 8) - 2)];
            if (ot == OT_LONG) {
        ...
        case 0x1c4:
            s->rip_offset = 1;
            easton_gen_ldst_modrm(s, modrm, OT_WORD, OR_TMP0, 0);
            val = ldub_code(s->pc++);
            if (b1) {
                val &= 7;
                //tcg_//gen_st16_tl(cpu_T[0], cpu_env,
                                //offsetof(CPUX86State,xmm_regs[reg].XMM_W(val)));
            } else {
                val &= 3;
                //tcg_//gen_st16_tl(cpu_T[0], cpu_env,
        ...
        case 0x1c5:
            ...
            if (b1) {
                val &= 7;
                rm = (modrm & 7) | REX_B(s);
                //tcg_//gen_ld16u_tl(cpu_T[0], cpu_env,
                                 //offsetof(CPUX86State,xmm_regs[rm].XMM_W(val)));
            } else {
                val &= 3;
                rm = (modrm & 7);
        ...
        case 0x1d7:
            ...
            if (b1) {
                rm = (modrm & 7) | REX_B(s);
                //tcg_//gen_addi_ptr(cpu_ptr0, cpu_env, offsetof(CPUX86State,xmm_regs[rm]));
                //gen_helper_pmovmskb_xmm(cpu_tmp2_i32, cpu_ptr0);
            } else {
                rm = (modrm & 7);
                //tcg_//gen_addi_ptr(cpu_ptr0, cpu_env, offsetof(CPUX86State,fpregs[rm].mmx));
                //gen_helper_pmovmskb_mmx(cpu_tmp2_i32, cpu_ptr0);
            }
            //tcg_//gen_extu_i32_tl(cpu_T[0], cpu_tmp2_i32);
            reg = ((modrm >> 3) & 7) | rex_r;
        ...
        case 0x038:
            ...
            if (b1) {
                ...
                if (mod == 3) {
                    ...
                    case 0x21: case 0x31: /* pmovsxbd, pmovzxbd */
                    case 0x24: case 0x34: /* pmovsxwq, pmovzxwq */
                        //tcg_//gen_qemu_ld32u(cpu_tmp0, cpu_A0,
                                          //(s->mem_index >> 2) - 1);
                        //tcg_//gen_trunc_tl_i32(cpu_tmp2_i32, cpu_tmp0);
                        //tcg_//gen_st_i32(cpu_tmp2_i32, cpu_env, op2_offset +
                                        //offsetof(XMMReg, XMM_L(0)));
                        break;
                    case 0x22: case 0x32: /* pmovsxbq, pmovzxbq */
                        //tcg_//gen_qemu_ld16u(cpu_tmp0, cpu_A0,
                                          //(s->mem_index >> 2) - 1);
                        //tcg_//gen_st16_tl(cpu_tmp0, cpu_env, op2_offset +
                                        //offsetof(XMMReg, XMM_W(0)));
                        break;
                    case 0x2a:            /* movntqda */
                        //gen_ldo_env_A0(s->mem_index, op1_offset);
        ...
        case 0x13a:
            ...
            if (sse_op2 == SSE_SPECIAL) {
                ...
                switch (b) {
                case 0x14: /* pextrb */
                    //tcg_//gen_ld8u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,
                                            //xmm_regs[reg].XMM_B(val & 15)));
                    //if (mod == 3)
                        //gen_op_mov_reg_T0(ot, rm);
                    //else
                        //tcg_//gen_qemu_st8(cpu_T[0], cpu_A0,
                                        //(s->mem_index >> 2) - 1);
                    break;
                case 0x15: /* pextrw */
                    //tcg_//gen_ld16u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,
                                           // xmm_regs[reg].XMM_W(val & 7)));
                    //if (mod == 3)
                        //gen_op_mov_reg_T0(ot, rm);
                    //else
                        //tcg_//gen_qemu_st16(cpu_T[0], cpu_A0,
                                        //(s->mem_index >> 2) - 1);
                    break;
                case 0x16:
                    if (ot == OT_LONG) { /* pextrd */
                        //tcg_//gen_ld_i32(cpu_tmp2_i32, cpu_env,
                                        //offsetof(CPUX86State,
                                                //xmm_regs[reg].XMM_L(val & 3)));
                        //tcg_//gen_extu_i32_tl(cpu_T[0], cpu_tmp2_i32);
                        //if (mod == 3)
                            //gen_op_mov_reg_v(ot, rm, cpu_T[0]);
                        //else
                            //tcg_//gen_qemu_st32(cpu_T[0], cpu_A0,
                                            //(s->mem_index >> 2) - 1);
                    } else { /* pextrq */
#ifdef TARGET_X86_64
                        //tcg_//gen_ld_i64(cpu_tmp1_i64, cpu_env,
                        ...
                        //if (mod == 3)
                            //gen_op_mov_reg_v(ot, rm, cpu_tmp1_i64);
                        //else
                            //tcg_//gen_qemu_st64(cpu_tmp1_i64, cpu_A0,
                                            //(s->mem_index >> 2) - 1);
#else
                        goto illegal_op;
#endif
                ...
                case 0x20: /* pinsrb */
                    //if (mod == 3)
                        //gen_op_mov_TN_reg(OT_LONG, 0, rm);
                    //else
                        //tcg_//gen_qemu_ld8u(cpu_tmp0, cpu_A0,
                                        //(s->mem_index >> 2) - 1);
                    //tcg_//gen_st8_tl(cpu_tmp0, cpu_env, offsetof(CPUX86State,
                                            //xmm_regs[reg].XMM_B(val & 15)));
                    break;
                case 0x21: /* insertps */
                    if (mod == 3) {
                    ...
                    //tcg_//gen_st_i32(cpu_tmp2_i32, cpu_env,
                                    //offsetof(CPUX86State,xmm_regs[reg]
                                            //.XMM_L((val >> 4) & 3)));
                    //if ((val >> 0) & 1)
                        //tcg_//gen_st_i32(//tcg_const_i32(0 /*float32_zero*/),
                                        //cpu_env, offsetof(CPUX86State,
                                                //xmm_regs[reg].XMM_L(0)));
                    //if ((val >> 1) & 1)
                        //tcg_//gen_st_i32(//tcg_const_i32(0 /*float32_zero*/),
                ...
                case 0x22:
                    if (ot == OT_LONG) { /* pinsrd */
                        ...
                        //if (mod == 3)
                            //gen_op_mov_v_reg(ot, cpu_tmp1_i64, rm);
                        //else
                            //tcg_//gen_qemu_ld64(cpu_tmp1_i64, cpu_A0,
                                            //(s->mem_index >> 2) - 1);
                        //tcg_//gen_st_i64(cpu_tmp1_i64, cpu_env,
                                        //offsetof(CPUX86State,
                                                //xmm_regs[reg].XMM_Q(val & 1)));
...
int calc_insn_length(DisasContext *s, target_ulong pc_start)
{
    ...
    //if (unlikely(qemu_loglevel_mask(CPU_LOG_TB_OP)))
        //tcg_//gen_debug_insn_start(pc_start);
    s->pc = pc_start;
    prefixes = 0;
    aflag = s->code32;
    dflag = s->code32;
 ...
 reswitch:
    ...
    case 0xf7:
        ...
        case 2: /* not */
            ...
            //tcg_//gen_not_tl(cpu_T[0], cpu_T[0]);
            if (mod != 3) {
                //gen_op_st_T0_A0(ot + s->mem_index);
            } else {
                //gen_op_mov_reg_T0(ot, rm);
        ...
        case 3: /* neg */
            ...
            //tcg_//gen_neg_tl(cpu_T[0], cpu_T[0]);
            if (mod != 3) {
                //gen_op_st_T0_A0(ot + s->mem_index);
            } else {
                //gen_op_mov_reg_T0(ot, rm);
        ...
        case 4: /* mul */
            ...
            case OT_BYTE:
                //gen_op_mov_TN_reg(OT_BYTE, 1, R_EAX);
                //tcg_//gen_ext8u_tl(cpu_T[0], cpu_T[0]);
                //tcg_//gen_ext8u_tl(cpu_T[1], cpu_T[1]);
                /* XXX: use 32 bit mul which could be faster */
                //tcg_//gen_mul_tl(cpu_T[0], cpu_T[0], cpu_T[1]);
                //gen_op_mov_reg_T0(OT_WORD, R_EAX);
                //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                //tcg_//gen_andi_tl(cpu_cc_src, cpu_T[0], 0xff00);
                s->cc_op = CC_OP_MULB;
                break;
            case OT_WORD:
                //gen_op_mov_TN_reg(OT_WORD, 1, R_EAX);
                //tcg_//gen_ext16u_tl(cpu_T[0], cpu_T[0]);
                //tcg_//gen_ext16u_tl(cpu_T[1], cpu_T[1]);
                /* XXX: use 32 bit mul which could be faster */
                //tcg_//gen_mul_tl(cpu_T[0], cpu_T[0], cpu_T[1]);
                //gen_op_mov_reg_T0(OT_WORD, R_EAX);
                //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                //tcg_//gen_shri_tl(cpu_T[0], cpu_T[0], 16);
                //gen_op_mov_reg_T0(OT_WORD, R_EDX);
                //tcg_//gen_mov_tl(cpu_cc_src, cpu_T[0]);
                s->cc_op = CC_OP_MULW;
                break;
            default:
            case OT_LONG:
#ifdef TARGET_X86_64
                //gen_op_mov_TN_reg(OT_LONG, 1, R_EAX);
                //tcg_//gen_ext32u_tl(cpu_T[0], cpu_T[0]);
                //tcg_//gen_ext32u_tl(cpu_T[1], cpu_T[1]);
                //tcg_//gen_mul_tl(cpu_T[0], cpu_T[0], cpu_T[1]);
                //gen_op_mov_reg_T0(OT_LONG, R_EAX);
                //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                //tcg_//gen_shri_tl(cpu_T[0], cpu_T[0], 32);
                //gen_op_mov_reg_T0(OT_LONG, R_EDX);
                //tcg_//gen_mov_tl(cpu_cc_src, cpu_T[0]);
#else
                {
                    ...
                    t1 = tcg_temp_new_i64();
                    //gen_op_mov_TN_reg(OT_LONG, 1, R_EAX);
                    //tcg_//gen_extu_i32_i64(t0, cpu_T[0]);
                    //tcg_//gen_extu_i32_i64(t1, cpu_T[1]);
                    //tcg_//gen_mul_i64(t0, t0, t1);
                    //tcg_//gen_trunc_i64_i32(cpu_T[0], t0);
                    //gen_op_mov_reg_T0(OT_LONG, R_EAX);
                    //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                    //tcg_//gen_shri_i64(t0, t0, 32);
                    //tcg_//gen_trunc_i64_i32(cpu_T[0], t0);
                    //gen_op_mov_reg_T0(OT_LONG, R_EDX);
                    //tcg_//gen_mov_tl(cpu_cc_src, cpu_T[0]);
                }
        ...
        case 5: /* imul */
            ...
            case OT_BYTE:
                //gen_op_mov_TN_reg(OT_BYTE, 1, R_EAX);
                //tcg_//gen_ext8s_tl(cpu_T[0], cpu_T[0]);
                //tcg_//gen_ext8s_tl(cpu_T[1], cpu_T[1]);
                /* XXX: use 32 bit mul which could be faster */
                //tcg_//gen_mul_tl(cpu_T[0], cpu_T[0], cpu_T[1]);
                //gen_op_mov_reg_T0(OT_WORD, R_EAX);
                //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                //tcg_//gen_ext8s_tl(cpu_tmp0, cpu_T[0]);
                //tcg_//gen_sub_tl(cpu_cc_src, cpu_T[0], cpu_tmp0);
                s->cc_op = CC_OP_MULB;
                break;
            case OT_WORD:
                //gen_op_mov_TN_reg(OT_WORD, 1, R_EAX);
                //tcg_//gen_ext16s_tl(cpu_T[0], cpu_T[0]);
                //tcg_//gen_ext16s_tl(cpu_T[1], cpu_T[1]);
                /* XXX: use 32 bit mul which could be faster */
                //tcg_//gen_mul_tl(cpu_T[0], cpu_T[0], cpu_T[1]);
                //gen_op_mov_reg_T0(OT_WORD, R_EAX);
                //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                //tcg_//gen_ext16s_tl(cpu_tmp0, cpu_T[0]);
                //tcg_//gen_sub_tl(cpu_cc_src, cpu_T[0], cpu_tmp0);
                //tcg_//gen_shri_tl(cpu_T[0], cpu_T[0], 16);
                //gen_op_mov_reg_T0(OT_WORD, R_EDX);
                s->cc_op = CC_OP_MULW;
            ...
            case OT_LONG:
#ifdef TARGET_X86_64
                //gen_op_mov_TN_reg(OT_LONG, 1, R_EAX);
                //tcg_//gen_ext32s_tl(cpu_T[0], cpu_T[0]);
                //tcg_//gen_ext32s_tl(cpu_T[1], cpu_T[1]);
                //tcg_//gen_mul_tl(cpu_T[0], cpu_T[0], cpu_T[1]);
                //gen_op_mov_reg_T0(OT_LONG, R_EAX);
                //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                //tcg_//gen_ext32s_tl(cpu_tmp0, cpu_T[0]);
                //tcg_//gen_sub_tl(cpu_cc_src, cpu_T[0], cpu_tmp0);
                //tcg_//gen_shri_tl(cpu_T[0], cpu_T[0], 32);
                //gen_op_mov_reg_T0(OT_LONG, R_EDX);
#else
                {
                    ...
                    t1 = tcg_temp_new_i64();
                    //gen_op_mov_TN_reg(OT_LONG, 1, R_EAX);
                    //tcg_//gen_ext_i32_i64(t0, cpu_T[0]);
                    //tcg_//gen_ext_i32_i64(t1, cpu_T[1]);
                    //tcg_//gen_mul_i64(t0, t0, t1);
                    //tcg_//gen_trunc_i64_i32(cpu_T[0], t0);
                    //gen_op_mov_reg_T0(OT_LONG, R_EAX);
                    //tcg_//gen_mov_tl(cpu_cc_dst, cpu_T[0]);
                    //tcg_//gen_sari_tl(cpu_tmp0, cpu_T[0], 31);
                    //tcg_//gen_shri_i64(t0, t0, 32);
                    //tcg_//gen_trunc_i64_i32(cpu_T[0], t0);
                    //gen_op_mov_reg_T0(OT_LONG, R_EDX);
                    //tcg_//gen_sub_tl(cpu_cc_src, cpu_T[0], cpu_tmp0);
    ...
    case 0x99: /* CDQ/CWD */
        ...
        if (dflag == 2) {
            //gen_op_mov_TN_reg(OT_QUAD, 0, R_EAX);
            //tcg_//gen_sari_tl(cpu_T[0], cpu_T[0], 63);
            //gen_op_mov_reg_T0(OT_QUAD, R_EDX);
        } else
#endif
        if (dflag == 1) {
            //gen_op_mov_TN_reg(OT_LONG, 0, R_EAX);
            //tcg_//gen_ext32s_tl(cpu_T[0], cpu_T[0]);
            //tcg_//gen_sari_tl(cpu_T[0], cpu_T[0], 31);
            //gen_op_mov_reg_T0(OT_LONG, R_EDX);
        } else {
            //gen_op_mov_TN_reg(OT_WORD, 0, R_EAX);
            //tcg_//gen_ext16s_tl(cpu_T[0], cpu_T[0]);
            //tcg_//gen_sari_tl(cpu_T[0], cpu_T[0], 15);
            //gen_op_mov_reg_T0(OT_WORD, R_EDX);
        }
        break;
    case 0x1af: /* imul Gv, Ev */
    ...
    case 0x6b:
        ...
        if (ot == OT_LONG) {
...
#else
                {
                    ...
                    //tcg_//gen_sari_tl(cpu_tmp0, cpu_T[0], 31);
                    //tcg_//gen_shri_i64(t0, t0, 32);
                    //tcg_//gen_trunc_i64_i32(cpu_T[1], t0);
                    //tcg_//gen_sub_tl(cpu_cc_src, cpu_T[1], cpu_tmp0);
                }
#endif
        } else {
            //tcg_//gen_ext16s_tl(cpu_T[0], cpu_T[0]);
    ...
    case 0x1b1: /* cmpxchg Ev, Gv */
        {
            ...
            if (mod == 3) {
                ...
                easton_gen_lea_modrm(s, modrm, &reg_addr, &offset_addr);
                //tcg_//gen_mov_tl(a0, cpu_A0);
                //gen_op_ld_v(ot + s->mem_index, t0, a0);
                rm = 0; /* avoid warning */
            }
            //label1 = //gen_new_label();
            //tcg_//gen_sub_tl(t2, cpu_regs[R_EAX], t0);
            //gen_extu(ot, t2);
            //tcg_//gen_brcondi_tl(TCG_COND_EQ, t2, 0, label1);
            //if (mod == 3) {
                //label2 = //gen_new_label();
                //gen_op_mov_reg_v(ot, R_EAX, t0);
                //tcg_//gen_br(label2);
                ...
                //gen_set_label(label2);
            //} else {
                //tcg_//gen_mov_tl(t1, t0);
                //gen_op_mov_reg_v(ot, R_EAX, t0);
                //gen_set_label(label1);
                /* always store */
                //gen_op_st_v(ot + s->mem_index, t1, a0);
            //}
            //tcg_//gen_mov_tl(cpu_cc_src, t0);
            //tcg_//gen_mov_tl(cpu_cc_dst, t2);
            s->cc_op = CC_OP_SUBB + ot;
            tcg_temp_free(t0);
            tcg_temp_free(t1);
            tcg_temp_free(t2);
    ...
    case 0x8f: /* pop Ev */
        ...
        if (mod == 3) {
            ...
            /* NOTE: order is important too for MMU exceptions */
            s->popl_esp_hack = 1 << ot;
            easton_gen_ldst_modrm(s, modrm, ot, OR_TMP0, 1);
            s->popl_esp_hack = 0;
            //gen_pop_update(s);
        }
        break;
    ...
    case 0x89: /* mov Gv, Ev */
        ...
        /* //generate a //generic store */
        easton_gen_ldst_modrm(s, modrm, ot, reg, 1);
        break;
    case 0xc6:
    case 0xc7: /* mov Ev, Iv */
        if ((b & 1) == 0)
    ...
    case 0xd7: /* xlat */
        ...
        //if (s->aflag == 2) {
            //gen_op_movq_A0_reg(R_EBX);
            //gen_op_mov_TN_reg(OT_QUAD, 0, R_EAX);
            //tcg_//gen_andi_tl(cpu_T[0], cpu_T[0], 0xff);
            //tcg_//gen_add_tl(cpu_A0, cpu_A0, cpu_T[0]);
        //} else
#endif
        //{
            //gen_op_movl_A0_reg(R_EBX);
            ...
            //if (s->aflag == 0)
                //gen_op_andl_A0_ffff();
            //else
                //tcg_//gen_andi_tl(cpu_A0, cpu_A0, 0xffffffff);
        //}
        //gen_add_A0_ds_seg(s);
        //gen_op_ldu_T0_A0(OT_BYTE + s->mem_index);
        //gen_op_mov_reg_T0(OT_BYTE, R_EAX);
    ...
    do_shiftd:
        ...
        if (shift) {
            val = ldub_code(s->pc++);
            //tcg_//gen_movi_tl(cpu_T3, val);
        } else {
            //tcg_//gen_mov_tl(cpu_T3, cpu_regs[R_ECX]);
        }
        //gen_shiftd_rm_T1_T3(s, ot, opreg, op);
        break;
    ...
    case 0xe7:
        ...
        //if (use_icount)
            //gen_io_start();
        //tcg_//gen_trunc_tl_i32(cpu_tmp2_i32, cpu_T[0]);
        //tcg_//gen_andi_i32(cpu_tmp2_i32, cpu_tmp2_i32, 0xffff);
        //tcg_//gen_trunc_tl_i32(cpu_tmp3_i32, cpu_T[1]);
        //gen_helper_out_func(ot, cpu_tmp2_i32, cpu_tmp3_i32);
        if (use_icount) {
            //gen_io_end();
            //gen_jmp(s, s->pc - s->cs_base);
    ...
    case 0x190 ... 0x19f: /* setcc Gv */
        modrm = ldub_code(s->pc++);
        //gen_setcc(s, b);
        easton_gen_ldst_modrm(s, modrm, OT_BYTE, OR_TMP0, 1);
        break;
    case 0x140 ... 0x14f: /* cmov Gv, Ev */
        {
            int l1;
            ...
            if (ot == OT_LONG) {
                ...
                //l1 = //gen_new_label();
                //gen_jcc1(s, s->cc_op, b ^ 1, l1);
                //tcg_//gen_mov_tl(cpu_regs[reg], t0);
                //gen_set_label(l1);
                //tcg_//gen_ext32u_tl(cpu_regs[reg], cpu_regs[reg]);
            } else
#endif
            {
                //l1 = //gen_new_label();
    ...
    case 0x9e: /* sahf */
        ...
        //if (s->cc_op != CC_OP_DYNAMIC)
            //gen_op_set_cc_op(s->cc_op);
        //gen_compute_eflags(cpu_cc_src);
        //tcg_//gen_andi_tl(cpu_cc_src, cpu_cc_src, CC_O);
        //tcg_//gen_andi_tl(cpu_T[0], cpu_T[0], CC_S | CC_Z | CC_A | CC_P | CC_C);
        //tcg_//gen_or_tl(cpu_cc_src, cpu_cc_src, cpu_T[0]);
        s->cc_op = CC_OP_EFLAGS;
        break;
    case 0x9f: /* lahf */
        if (CODE64(s) && !(s->cpuid_ext3_features & CPUID_EXT3_LAHF_LM))
        ...
        //gen_compute_eflags(cpu_T[0]);
        /* Note: //gen_compute_eflags() only gives the condition codes */
        //tcg_//gen_ori_tl(cpu_T[0], cpu_T[0], 0x02);
        //gen_op_mov_reg_T0(OT_BYTE, R_AH);
        break;
    case 0xf5: /* cmc */
        //if (s->cc_op != CC_OP_DYNAMIC)
            //gen_op_set_cc_op(s->cc_op);
        //gen_compute_eflags(cpu_cc_src);
        //tcg_//gen_xori_tl(cpu_cc_src, cpu_cc_src, CC_C);
        s->cc_op = CC_OP_EFLAGS;
        break;
    case 0xf8: /* clc */
        //if (s->cc_op != CC_OP_DYNAMIC)
            //gen_op_set_cc_op(s->cc_op);
        //gen_compute_eflags(cpu_cc_src);
        //tcg_//gen_andi_tl(cpu_cc_src, cpu_cc_src, ~CC_C);
        s->cc_op = CC_OP_EFLAGS;
        break;
    case 0xf9: /* stc */
        //if (s->cc_op != CC_OP_DYNAMIC)
            //gen_op_set_cc_op(s->cc_op);
        //gen_compute_eflags(cpu_cc_src);
        //tcg_//gen_ori_tl(cpu_cc_src, cpu_cc_src, CC_C);
        s->cc_op = CC_OP_EFLAGS;
        break;
    case 0xfc: /* cld */
        //tcg_//gen_movi_i32(cpu_tmp2_i32, 1);
        //tcg_//gen_st_i32(cpu_tmp2_i32, cpu_env, offsetof(CPUState, df));
        break;
    case 0xfd: /* std */
        //tcg_//gen_movi_i32(cpu_tmp2_i32, -1);
        //tcg_//gen_st_i32(cpu_tmp2_i32, cpu_env, offsetof(CPUState, df));
    ...
    do_btx:
        ...
        if (mod != 3) {
            ...
            /* specific case: we need to add a displacement */
            //gen_exts(ot, cpu_T[1]);
            //tcg_//gen_sari_tl(cpu_tmp0, cpu_T[1], 3 + ot);
            //tcg_//gen_shli_tl(cpu_tmp0, cpu_tmp0, ot);
            //tcg_//gen_add_tl(cpu_A0, cpu_A0, cpu_tmp0);
            //gen_op_ld_T0_A0(ot + s->mem_index);
        } else {
            //gen_op_mov_TN_reg(ot, 0, rm);
        }
    bt_op:
        //tcg_//gen_andi_tl(cpu_T[1], cpu_T[1], (1 << (3 + ot)) - 1);
#ifdef CONFIG_COREMU
        if (s->prefix & PREFIX_LOCK) {
            //if (s->cc_op != CC_OP_DYNAMIC)
                //gen_op_set_cc_op(s->cc_op);
        ...
        switch(op) {
        case 0:
            //tcg_//gen_shr_tl(cpu_cc_src, cpu_T[0], cpu_T[1]);
            //tcg_//gen_movi_tl(cpu_cc_dst, 0);
            break;
        case 1:
            //tcg_//gen_shr_tl(cpu_tmp4, cpu_T[0], cpu_T[1]);
            //tcg_//gen_movi_tl(cpu_tmp0, 1);
            //tcg_//gen_shl_tl(cpu_tmp0, cpu_tmp0, cpu_T[1]);
            //tcg_//gen_or_tl(cpu_T[0], cpu_T[0], cpu_tmp0);
            break;
        case 2:
            //tcg_//gen_shr_tl(cpu_tmp4, cpu_T[0], cpu_T[1]);
            //tcg_//gen_movi_tl(cpu_tmp0, 1);
            //tcg_//gen_shl_tl(cpu_tmp0, cpu_tmp0, cpu_T[1]);
            //tcg_//gen_not_tl(cpu_tmp0, cpu_tmp0);
            //tcg_//gen_and_tl(cpu_T[0], cpu_T[0], cpu_tmp0);
            break;
        default:
        case 3:
            //tcg_//gen_shr_tl(cpu_tmp4, cpu_T[0], cpu_T[1]);
            //tcg_//gen_movi_tl(cpu_tmp0, 1);
            //tcg_//gen_shl_tl(cpu_tmp0, cpu_tmp0, cpu_T[1]);
            //tcg_//gen_xor_tl(cpu_T[0], cpu_T[0], cpu_tmp0);
            break;
        }
        s->cc_op = CC_OP_SARB + ot;
        if (op != 0) {
            ...
            //else
                //gen_op_mov_reg_T0(ot, rm);
            //tcg_//gen_mov_tl(cpu_cc_src, cpu_tmp4);
            //tcg_//gen_movi_tl(cpu_cc_dst, 0);
        }
        break;
    case 0x1bc: /* bsf */
    case 0x1bd: /* bsr */
        {
            ...
            if ((b & 1) && (prefixes & PREFIX_REPZ) &&
                ...
                case OT_LONG: //gen_helper_lzcnt(cpu_T[0], t0,
                    //tcg_const_i32(32)); break;
                case OT_QUAD: break;//gen_helper_lzcnt(cpu_T[0], t0,
                    //tcg_const_i32(64)); break;
                }
                //gen_op_mov_reg_T0(ot, reg);
            } else {
                //label1 = //gen_new_label();
                //tcg_//gen_movi_tl(cpu_cc_dst, 0);
                //tcg_//gen_brcondi_tl(TCG_COND_EQ, t0, 0, label1);
                if (b & 1) {
                    //gen_helper_bsr(cpu_T[0], t0);
                } else {
                    //gen_helper_bsf(cpu_T[0], t0);
                ...
                //gen_op_mov_reg_T0(ot, reg);
                //tcg_//gen_movi_tl(cpu_cc_dst, 1);
                //gen_set_label(label1);
                //tcg_//gen_discard_tl(cpu_cc_src);
                s->cc_op = CC_OP_LOGICB + ot;
            }
            tcg_temp_free(t0);
        }
    ...
    case 0x1c8 ... 0x1cf: /* bswap reg */
        ...
        if (dflag == 2) {
            //gen_op_mov_TN_reg(OT_QUAD, 0, reg);
            //tcg_//gen_bswap64_i64(cpu_T[0], cpu_T[0]);
            //gen_op_mov_reg_T0(OT_QUAD, reg);
        } else
#endif
        {
            //gen_op_mov_TN_reg(OT_LONG, 0, reg);
            //tcg_//gen_ext32u_tl(cpu_T[0], cpu_T[0]);
            //tcg_//gen_bswap32_tl(cpu_T[0], cpu_T[0]);
            //gen_op_mov_reg_T0(OT_LONG, reg);
        }
        break;
    case 0xd6: /* salc */
    ...
    case 0xe3: /* jecxz */
        {
            ...
            case 1: /* loopz */
                ...
                //gen_op_jz_ecx(s->aflag, l3);
                //gen_compute_eflags(cpu_tmp0);
                //tcg_//gen_andi_tl(cpu_tmp0, cpu_tmp0, CC_Z);
                if (b == 0) {
                    //tcg_//gen_brcondi_tl(TCG_COND_EQ, cpu_tmp0, 0, l1);
                } else {
                    //tcg_//gen_brcondi_tl(TCG_COND_NE, cpu_tmp0, 0, l1);
                }
                break;
            case 2: /* loop */
                //gen_op_add_reg_im(s->aflag, R_ECX, -1);
    ...
    case 0x100:
        ...
        case 0: /* sldt */
            if (!s->pe || s->vm86)
                goto illegal_op;
            //gen_svm_check_intercept(s, pc_start, SVM_EXIT_LDTR_READ);
            //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,ldt.selector));
            ot = OT_WORD;
            if (mod == 3)
                ot += s->dflag;
            easton_gen_ldst_modrm(s, modrm, ot, OR_TMP0, 1);
        ...
        case 1: /* str */
            if (!s->pe || s->vm86)
                goto illegal_op;
            //gen_svm_check_intercept(s, pc_start, SVM_EXIT_TR_READ);
            //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,tr.selector));
            ot = OT_WORD;
            if (mod == 3)
                ot += s->dflag;
            easton_gen_ldst_modrm(s, modrm, ot, OR_TMP0, 1);
    ...
    case 0x101:
        ...
        case 0: /* sgdt */
            ...
            //gen_svm_check_intercept(s, pc_start, SVM_EXIT_GDTR_READ);
            easton_gen_lea_modrm(s, modrm, &reg_addr, &offset_addr);
            //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, gdt.limit));
            //gen_op_st_T0_A0(OT_WORD + s->mem_index);
            //gen_add_A0_im(s, 2);
            //tcg_//gen_ld_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, gdt.base));
            //if (!s->dflag)
                //gen_op_andl_T0_im(0xffffff);
            //gen_op_st_T0_A0(CODE64(s) + OT_LONG + s->mem_index);
            break;
        case 1:
            if (mod == 3) {
                ...
                //gen_svm_check_intercept(s, pc_start, SVM_EXIT_IDTR_READ);
                easton_gen_lea_modrm(s, modrm, &reg_addr, &offset_addr);
                //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, idt.limit));
                //gen_op_st_T0_A0(OT_WORD + s->mem_index);
                //gen_add_A0_im(s, 2);
                //tcg_//gen_ld_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, idt.base));
                //if (!s->dflag)
                    //gen_op_andl_T0_im(0xffffff);
                //gen_op_st_T0_A0(CODE64(s) + OT_LONG + s->mem_index);
            }
        ...
        case 3: /* lidt */
            if (mod == 3) {
                ...
                //if (!s->dflag)
                    //gen_op_andl_T0_im(0xffffff);
                if (op == 2) {
                    //tcg_//gen_st_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,gdt.base));
                    //tcg_//gen_st32_tl(cpu_T[1], cpu_env, offsetof(CPUX86State,gdt.limit));
                } else {
                    //tcg_//gen_st_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,idt.base));
                    //tcg_//gen_st32_tl(cpu_T[1], cpu_env, offsetof(CPUX86State,idt.limit));
                }
            }
            break;
        case 4: /* smsw */
            //gen_svm_check_intercept(s, pc_start, SVM_EXIT_READ_CR0);
#if defined TARGET_X86_64 && defined HOST_WORDS_BI//genDIAN
            //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,cr[0]) + 4);
#else
            //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,cr[0]));
#endif
            easton_gen_ldst_modrm(s, modrm, OT_WORD, OR_TMP0, 1);
            break;
        case 6: /* lmsw */
            if (s->cpl != 0) {
                //gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
                ...
                case 0: /* swapgs */
                    ...
                    if (CODE64(s)) {
                        //if (s->cpl != 0) {
                            //gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
                        //} else {
                            //tcg_//gen_ld_tl(cpu_T[0], cpu_env,
                                //offsetof(CPUX86State,segs[R_GS].base));
                            //tcg_//gen_ld_tl(cpu_T[1], cpu_env,
                                //offsetof(CPUX86State,kernelgsbase));
                            //tcg_//gen_st_tl(cpu_T[1], cpu_env,
                                //offsetof(CPUX86State,segs[R_GS].base));
                            //tcg_//gen_st_tl(cpu_T[0], cpu_env,
                                //offsetof(CPUX86State,kernelgsbase));
                        //}
                    } else
#endif
    ...
    case 0x63: /* arpl or movslS (x86_64) */
...
#endif
        {
            ...
            //gen_op_mov_v_reg(ot, t1, reg);
            //tcg_//gen_andi_tl(cpu_tmp0, t0, 3);
            //tcg_//gen_andi_tl(t1, t1, 3);
            //tcg_//gen_movi_tl(t2, 0);
            //label1 = //gen_new_label();
            //tcg_//gen_brcond_tl(TCG_COND_GE, cpu_tmp0, t1, label1);
            //tcg_//gen_andi_tl(t0, t0, ~3);
            //tcg_//gen_or_tl(t0, t0, t1);
            //tcg_//gen_movi_tl(t2, CC_Z);
            //gen_set_label(label1);
            if (mod != 3) {
                //gen_op_st_v(ot + s->mem_index, t0, a0);
                tcg_temp_free(a0);
            ...
            //if (s->cc_op != CC_OP_DYNAMIC)
                //gen_op_set_cc_op(s->cc_op);
            //gen_compute_eflags(cpu_cc_src);
            //tcg_//gen_andi_tl(cpu_cc_src, cpu_cc_src, ~CC_Z);
            //tcg_//gen_or_tl(cpu_cc_src, cpu_cc_src, t2);
            s->cc_op = CC_OP_EFLAGS;
            tcg_temp_free(t0);
            tcg_temp_free(t1);
            tcg_temp_free(t2);
    ...
    case 0x103: /* lsl */
        {
            ...
            //else
                //gen_helper_lsl(t0, cpu_T[0]);
            //tcg_//gen_andi_tl(cpu_tmp0, cpu_cc_src, CC_Z);
            //label1 = //gen_new_label();
            //tcg_//gen_brcondi_tl(TCG_COND_EQ, cpu_tmp0, 0, label1);
            //gen_op_mov_reg_v(ot, reg, t0);
            //gen_set_label(label1);
            s->cc_op = CC_OP_EFLAGS;
            tcg_temp_free(t0);
    ...
    case 0x123: /* mov drN, reg */
        if (s->cpl != 0) {
            ...
            if (b & 2) {
                ...
                //gen_svm_check_intercept(s, pc_start, SVM_EXIT_READ_DR0 + reg);
                //tcg_//gen_ld_tl(cpu_T[0], cpu_env, offsetof(CPUX86State,dr[reg]));
                //gen_op_mov_reg_T0(ot, rm);
            }
        }
        break;
    ...
    case 0x1ae:
        ...
        case 3: /* stmxcsr */
            ...
            if (op == 2) {
                //gen_op_ld_T0_A0(OT_LONG + s->mem_index);
                //tcg_//gen_st32_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, mxcsr));
            } else {
                //tcg_//gen_ld32u_tl(cpu_T[0], cpu_env, offsetof(CPUX86State, mxcsr));
                //gen_op_st_T0_A0(OT_LONG + s->mem_index);
            }
            break;
        case 5: /* lfence */
    ...
    case 0x1c4 ... 0x1c6:
    case 0x1d0 ... 0x1fe:
        easton_gen_sse(s, b, pc_start, rex_r);
        break;
    default:
        goto illegal_op;
    }
    ...
    //easton added
    //call //gen_helper
    //there are two situations, this is 1
    ////gen_helper_tb_trace(tcg_const_i64(pc_start),tcg_const_i32(s->pc-pc_start));//old //gen_helper
    return s->pc-pc_start;
 illegal_op:
    //if (s->prefix & PREFIX_LOCK)
        //gen_helper_unlock();
    /* XXX: ensure that no lock was //generated */
    ...
    //easton added
    //call //gen_helper
    //there are two situations, this is 2
    ////gen_helper_tb_trace(tcg_const_i64(pc_start),tcg_const_i32(s->pc-pc_start));//old //gen_helper
    return s->pc-pc_start;
}

#ifdef ANALYTICAL_MODEL_TRANSLATE_FETCH_INSTR
void ana_translate_fetch_instr(target_ulong ana_start_pc, target_ulong ana_end_pc){
	target_ulong ana_helper_fetch_instr_index = ana_start_pc;
#ifdef ANALYTICAL_MODEL_TRACE_LOG
	//ftrifs_trace_log("aaa : %d %x %x\n", ana_single_tb_instr_index, ana_start_pc, ana_end_pc);
#endif
	ana_fetch_instr_temp_buffer[ana_single_tb_instr_index].ana_x86_virt_pc = ana_start_pc;
	int ana_single_instr_byte_index = 0;
	for(; ana_helper_fetch_instr_index < ana_end_pc; ana_helper_fetch_instr_index++){
		ana_fetch_instr_temp_buffer[ana_single_tb_instr_index].ana_x86_inst[ana_single_instr_byte_index] = (unsigned char)ldub_code(ana_helper_fetch_instr_index);
		ana_single_instr_byte_index++;
	}
	ana_single_tb_instr_index++;
}
#endif
                                                  ...
                                                  int search_pc)
{
...
#endif
    dc->flags = flags;
#ifdef ANALYTICAL_MODEL_COREMU_MODIFY
	if(1){
		dc->jmp_opt = 0;
	}
	else{
	...
	//if(core_analytical_model_instrument_flag_before_magic_break == 0){
	//	if(analytical_model_instrument_flag_before_magic_break){
	//		core_analytical_model_instrument_flag_before_magic_break = 1;
	//	}
	//}
	
	if(core_analytical_model_instrument_flag == 0){
		if(analytical_model_instrument_flag){
			core_analytical_model_instrument_flag = 1;
#ifdef ANALYTICAL_MODEL_TRACE_LOG
			ftrifs_trace_log("start: %d\n", env->cpu_index);
#endif
			//tb_flush(env);
			//printf("core %d start analytical model\n", env->cpu_index);
		}
	}
#endif
...
#ifdef ANALYTICAL_MODEL_FETCH_INSTR
	//if(core_analytical_model_instrument_flag){
		ana_record_pc = pc_ptr;
	//}
#endif
#ifdef ANALYTICAL_MODEL_HELPLER_FETCH_INSTR
	ana_helpler_record_pc = pc_ptr;
#ifdef ANALYTICAL_MODEL_TRANSLATE_FETCH_INSTR
	
#else
	DisasContext *easton_tmp_s=NULL;
	if(easton_tmp_s == NULL){
		easton_tmp_s=malloc(sizeof(DisasContext));
	}
	memcpy(easton_tmp_s,dc,sizeof(DisasContext));
	int insn_length = calc_insn_length(easton_tmp_s,pc_ptr);
	ana_helpler_record_next_pc = ana_helpler_record_pc + insn_length;
	gen_helper_fetch_instruction(tcg_const_tl(ana_helpler_record_pc), tcg_const_tl(ana_helpler_record_next_pc));
#endif
	//pc_ptr = ana_helpler_record_pc;
#endif
...
#ifdef ANALYTICAL_MODEL_FETCH_INSTR
	//if(core_analytical_model_instrument_flag){
		ana_record_next_pc = pc_ptr;
	//}
	if(core_analytical_model_instrument_flag){
		assert((ana_record_next_pc - ana_record_pc) < ANALYTICAL_INSTRUCTION_FETCH_SINGLE_MAX_LENTGH);
		func_analytical_single_instr_fetch(ana_record_pc, ana_record_next_pc);
	}
#endif
#ifdef ANALYTICAL_MODEL_HELPLER_FETCH_INSTR
	ana_helpler_record_next_pc = pc_ptr;
#ifdef ANALYTICAL_MODEL_TRANSLATE_FETCH_INSTR
	if(core_analytical_model_instrument_flag){
		ana_translate_fetch_instr(ana_helpler_record_pc, ana_helpler_record_next_pc);
	}
#endif
	/*int true_insn_length = ana_helpler_record_next_pc - ana_helpler_record_pc;
	if(insn_length != true_insn_length){
		printf("cal: 0x%llx disas: 0x%llx\n", ana_cal_print, ana_disas_print);
		printf("cal_last: %llx, true_last: %llx\n", ana_helpler_record_next_pc, pc_ptr);
		printf("****************aaa***********************: %d %d\n", true_insn_length, insn_length);
		exit(0);
	}// */
	//if(core_analytical_model_instrument_flag){
	//}
	//gen_helper_fetch_instruction(tcg_const_tl(0), tcg_const_tl(0));
#endif
