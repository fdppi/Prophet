...
static COREMU_THREAD uint8_t *code_gen_ptr;
...
int analytical_model_instrument_flag_before_magic_break = 0;
COREMU_THREAD int core_analytical_model_instrument_flag_before_magic_break;
int analytical_model_instrument_flag = 0;
COREMU_THREAD int core_analytical_model_instrument_flag;
COREMU_THREAD int previous_timing_call;

#ifdef ANALYTICAL_MODEL_STATISTICS
uint64_t analytical_user_instruction_num[ANALYTICAL_COREMU_TOTAL_CORE];
uint64_t analytical_kernel_instruction_num[ANALYTICAL_COREMU_TOTAL_CORE];
uint64_t analytical_total_instruction_num[ANALYTICAL_COREMU_TOTAL_CORE];
int timing_call_times[ANALYTICAL_COREMU_TOTAL_CORE];
uint64_t analytical_max_basic_block_instr_num[ANALYTICAL_COREMU_TOTAL_CORE];

// ANALYTICAL_COREMU_THREAD_VARIABLES
COREMU_THREAD int analytical_user_level_statics;
COREMU_THREAD FILE *fanalytical_log_file;

    //#define minqh_timer_count
    #ifdef minqh_timer_count
    COREMU_THREAD int analytical_core_interrupted;
    uint64_t analytical_interrupt_count[ANALYTICAL_COREMU_TOTAL_CORE];
    uint64_t analytical_ipi_count[ANALYTICAL_COREMU_TOTAL_CORE];
    uint64_t analytical_ipi_send_count[ANALYTICAL_COREMU_TOTAL_CORE];
    #endif

#endif

#ifdef ANALYTICAL_MODEL_FETCH_INSTR
COREMU_THREAD unsigned char fetch_single_instr_bytes[ANALYTICAL_INSTRUCTION_FETCH_SINGLE_MAX_LENTGH];
COREMU_THREAD target_ulong ana_record_pc;
COREMU_THREAD target_ulong ana_record_next_pc;
//COREMU_THREAD map< uint64_t, code_cache* > bb_cache_map;
#endif

#ifdef ANALYTICAL_MODEL_HELPLER_FETCH_INSTR
COREMU_THREAD target_ulong ana_helpler_record_pc;
COREMU_THREAD target_ulong ana_helpler_record_next_pc;
COREMU_THREAD ana_instr_struct_t *ana_fetch_instr_temp_buffer;
COREMU_THREAD int ana_single_tb_instr_index;
#endif

#ifdef ANALYTICAL_MODEL_HELPLER_FETCH_MEM
COREMU_THREAD ana_mem_struct_t *ana_fetch_mem_temp_buffer;
COREMU_THREAD int ana_single_tb_mem_index;
COREMU_THREAD target_ulong ana_mem_phy_page_log[ANA_MEM_PHY_PAGE_LOG_SIZE];
COREMU_THREAD target_ulong ana_mem_phy_page_addr_log[ANA_MEM_PHY_PAGE_LOG_SIZE];
COREMU_THREAD int ana_mem_phy_page_log_index;
COREMU_THREAD target_ulong ana_disas_print;
COREMU_THREAD target_ulong ana_cal_print;
#endif

#ifdef ANALYTICAL_MODEL_TIMING_SHIFT
COREMU_THREAD ana_tb_struct_t *ana_tb_temp_buffer;
#include "../ana_timing.h"
#endif
#endif
...
#ifndef CONFIG_COREMU
            /* Multiple threads may call this function. Disable this
             * optimization in COREMU. */
            QLIST_REMOVE(block, next);
